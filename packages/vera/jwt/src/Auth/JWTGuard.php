...
use Vera\JWT\Builder;
use Vera\JWT\Contracts\IClaimProvider;
use Vera\JWT\Exception\BaseJWTException;
use Vera\JWT\Exception\TokenNotInRequestException;
use Vera\JWT\Token;
use Vera\JWT\Util\CacheUtil;
use Vera\JWT\Util\ConfigUtil;
use Vera\JWT\Util\Parser;
...
class JWTGuard implements Guard
{
    ...
    private function fromUser(IClaimProvider $user)
    {
        ...
        $token = $builder->algoId($agloId)
            ...
            ->jwtId($jwtId)
            ->secretKey($secretKey)
            ->publicKey(ConfigUtil::getPublicSecretKey())
            ->privateKey(ConfigUtil::getPrivateSecretKey())
            ->build();
